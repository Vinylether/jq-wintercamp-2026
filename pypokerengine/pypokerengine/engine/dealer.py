  ...
  def process_message(self, address, msg):
    ...
    for receiver in receivers:
      if msg["type"] == 'ask':
        ...
        except func_timeout.exceptions.FunctionTimedOut:  # player timeout
          print('"%s" respond to ask timeout, automatically fold.' % type(receiver).__name__)
          return "fold", 0
      elif msg["type"] == 'notification':
        try:
          receiver.receive_notification(msg["message"])
        except func_timeout.exceptions.FunctionTimedOut:
          print('"%s" respond to notification timeout.' % type(receiver).__name__)
      else:
